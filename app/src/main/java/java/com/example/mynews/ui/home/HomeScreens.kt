...
import java.com.example.mynews.ui.article.postContentItems
import java.com.example.mynews.ui.article.sharePost
import java.com.example.mynews.ui.components.NyNewsSnackbarHost
import java.com.example.mynews.ui.modifiers.interceptKey
...
private fun HomeScreenWithList(
    ...
    Scaffold(
        snackbarHost = { NyNewsSnackbarHost(hostState = snackbarHostState) },
        topBar = {
            if (showTopAppBar) {
                HomeTopAppBar(
                    openDrawer = openDrawer,
...
private fun PostList(
    ...
    LazyColumn(
        ...
        item { PostListTopSection(postsFeed.selectedPost, onArticleTapped) }
        if (postsFeed.recommendedPost.isNotEmpty()) {
            item {
                PostListSimpleSection(
                    postsFeed.recommendedPost,
                    onArticleTapped,
                    favorites,
                    onToggleFavorite,
                )
    ...
    CenterAlignedTopAppBar(
        ...
        navigationIcon = {
            IconButton(onClick = openDrawer) {
                Icon(
                    painter = painterResource(R.drawable.ic_mynews_logo),
...
fun PreviewHomeListDrawerScreen() {
    ...
    MyNewsTheme {
        HomeFeedScreen(
            uiState = HomeUiState.HasPosts(
                postsFeed = postsFeed,
                selectedPost = postsFeed.selectedPost,
                isArticleOpen = false,
                favorites = emptySet(),
                isLoading = false,
